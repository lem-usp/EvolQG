...
test_that("MonteCarloRep returns sensible results",
          {
            expect_that(MonteCarloRep(RandomMatrix(10), RandomSkewers, 30, iterations = 1), gives_warning("Matrix appears to be a correlation matrix! Only covariance matrices should be used in parametric resampling."))
            
            cov.matrix <- RandomMatrix(10, 1, 1, 10)
            expect_that(MonteCarloRep(cov.matrix, RandomSkewers, 30, iterations = 50) <= 1, is_true())
            expect_that(MonteCarloRep(cov.matrix, MatrixCor, 30, correlation = TRUE) <= 1, is_true())
            expect_that(MonteCarloRep(cov.matrix, KrzCor, 30) <= 1, is_true())
            expect_that(MonteCarloRep(cov.matrix, KrzCor, 30, correlation = TRUE) <= 1, is_true())
          }
)
